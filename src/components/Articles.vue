      ...
      <div class='card p-2'>
          <div class='row'>
            <header class="col-md-9 d-none d-sm-block">
              <h1 class='fs-4 text-uppercase text-primary card-title'>{{ post.title.$t }}</h1> 
              <span class='fs-5 badge bg-secondary'>{{ filterLabels(post.category) }}</span>
            </header>
            <div class="col-md-3" style="background: linear-gradient(0deg, rgba(231,236,216,1) 0%, rgba(255,255,255,1) 27%)">
              <img class='w-100 rounded-circle' v-bind='{alt: post.title, src: filterPostImages(post.media$thumbnail)}' style="width:100%; height:100%; object-fit:contain" />
            </div>
            <div class="col-md-9 bg-light d-block d-sm-none d-flex justify-content-between align-items-center">
              <h1 class='m-0 text-secondary fs-6 fw-bold'>{{ post.title.$t }}</h1>
              <router-link class="btn" :to="{ name: 'Product', params: {id: post.id.$t} }">
  ...
  methods: {
    ...
    filterPostImages( evt ){
      if(!evt){
        ...
        return evt.url.replace("s1024","s320")
      }
    },
    showPostModal(evt){
      this.obj = evt;
